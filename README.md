...
| 文法 | 24 トピック | 冠詞、現在完了、前置詞、仮定法など、日本人がつまずきやすい項目。各トピックに確認クイズ5問 |

## 機能

- **単語カード・フレーズカード**：表を見て思い出し、タップで答えを確認。裏には意味と一緒にカタカナ読みが出ます（強く読む部分は【】で囲んであります）。「覚えた / まだ」で自己評価します。
- **間隔反復（SRS）**：「覚えた」カードは 2→4→7→15 日後に（初見で「覚えた」なら2日後から）、「まだ」のカードはその日のうちにもう一度出ます。期日が来た復習カードを優先し、残りの枠に新しいカードがランダムに入ります。出題順もシャッフルします。
- **文法ミニ講座**：1画面ずつ読める短い解説と、読み終わったあとの確認クイズ。
- **4択クイズ**：6種類から選べます。ミックス／単語の意味・つづり／例文の穴埋め・和訳／フレーズの意味・言い方／文法／アクセント（どこを強く読むか）。問題はランダムに出て、学習したことのあるカードが優先されます。
- **学習記録**：今日の学習数、連続学習日数、覚えた数（最後に「覚えた」を選んだカード）、定着した数、クイズの正答率。使い方の画面の右上にバージョンが出ます。
...
単語とフレーズは、`src/data/kana.ts` にカタカナ読みも足します（キーは英語そのまま、強く読む部分を【】で囲む）。
...
- カタカナ読みが抜けていないか（アクセントの【】があり、1行に収まるか）
- ID や英語の重複が無いか
